...
import algosdk
from shippingApp.algobotHandler.contract import Contract
...
def delivery_tx(b, tx, ctx=None):
    payment = tx.get('payment')
    if not payment:
        return
    if payment.get("to") != "X6E3EZE53KAJRPUWPRLRPRDIGIFIUUVB67LJ4RBA73GJDL5N7DRQ466RAM":
        return

    # get note field
    note = json.loads(base64.b64decode(tx.get("noteb64")))
    station = note['s']
    temperature = note['t']
    ...
    if ctx is None:
        return
    c = Contract(ctx["algod"])

    try:
        tx = c.execute("6QX7IOT2AVB7SILICWOUGMKDRVDBZFRFJTCSPZGGSWBRTGTQQXA56NOWNA", station, temperature)
        print(tx)
    except:
        pass

    try:
        tx = c.execute("PCGYCFY5BL3SBTPJNA56FVA4K2VJ75UCWW7LAV6QPRMJVHXEK4LJHYKKKE", station, temperature)
        print(tx)
    except:
        pass
